...
const addNewList = (event) => {
  ...
  // console.log(selectService.value, quantity.value);
  ...
  if (isExistedProduct) {
    ...
    const ru = isExistedProduct.querySelector(".row-unitprice");

    rq.innerText = parseFloat(rq.innerText) + quantity.valueAsNumber;
    console.log(rq.innerText);
    rt.innerText = rq.innerText * parseFloat(ru.innerText);
  } else {
    rows.append(createNewList(currentProduct, quantity.valueAsNumber));
  }
...
const createNewList = (product, quantity) => {
  ...
  tr.innerHTML = `
    ...
    <td class=" text-end row-unitprice"> ${product.price}</td>
    <td class=" text-end row-total"> ${amount.toFixed(2)}</td>
  `;
  return tr;
};
...
const calculateTotal = () => {
  let subTotalAmount = [...document.querySelectorAll(".row-total")].reduce(
...
app.addEventListener("click", (event) => {
  if (event.target.classList.contains("del-btn")) {
    if (confirm("Are you sure to delete this item from your list?")) {
...
currentDate.innerText = curDate.toISOString();
